...
export async function assertPublicUrl(
  ...
  const hostname = url.hostname; // '[::1]' for IPv6, 'example.com' otherwise
  const bare = hostname.replace(/^\[|\]$/g, ''); // '::1' or 'example.com'

  // IP literal — the URL parser normalises decimal/octal/hex IPv4 and
  // IPv4-mapped IPv6 into canonical notation before we see it. IPv6
  // literals keep their brackets in `hostname`, so strip them for net.isIP.
  const literalFamily = net.isIP(bare);
  if (literalFamily) {
    if (isBlockedAddress(bare)) {
      throw new BlockedUrlError(`blocked IP literal ${bare}`, 'ip');
    }
    return {
      url,
      addresses: [{ address: bare, family: literalFamily as 4 | 6 }],
    };
  }

  const records = await dns.lookup(hostname, { all: true, verbatim: true });
  if (!records.length) {
    throw new BlockedUrlError(`no DNS records for ${hostname}`, 'dns');
  }
  ...
  if (blocked) {
    throw new BlockedUrlError(
      `${hostname} resolves to blocked address ${blocked.address}`,
      'ip',
    );
  }
